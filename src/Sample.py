...
class Sample:
    ...
    def getStorageVersion(self):
        if(self.version_storeada!=None): return self.version_storeada
        if(self.version_try_to_load):
            self.version_try_to_load=False
            if(self.vc==None):return None
    ...
    def getBinary(self):
        ...
        if(self.pc==None):
            tmp_pc=PackageController() #we use a temporary PackageController so we don't leave a mongo cursor open.
