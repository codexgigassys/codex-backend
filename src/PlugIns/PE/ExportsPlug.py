...
class ExportsPlug(PlugIn):
    ...
    def process(self):
        ...
        if not hasattr(pelib, 'DIRECTORY_ENTRY_EXPORT'): return ret
        ...
        for export in pelib.DIRECTORY_ENTRY_EXPORT.symbols:
            ...
            if(export.forwarder!=None):
                if(export.forwarder.find('.')!=-1):
                    symbol["forwarder_dll"]=repr(str(export.forwarder).lower().split('.')[0]+".dll")
                    symbol["forwarder_function"]=repr(str(export.forwarder).lower().split('.')[1])
