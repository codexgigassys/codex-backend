...
class HeadersPlug(PlugIn):
    ...
    def process(self):
        ...
        fh["PointerToSymbolTable"]=self._normalize(pelib.FILE_HEADER.PointerToSymbolTable)
        fh["NumberOfSymbols"]=self._normalize(pelib.FILE_HEADER.NumberOfSymbols)
        fh["SizeOfOptionalHeader"]=self._normalize(pelib.FILE_HEADER.SizeOfOptionalHeader)
        fh["Characteristics"]=self._normalize(pelib.FILE_HEADER.Characteristics)

        oh={}
        oh["Magic"]=self._normalize(pelib.OPTIONAL_HEADER.Magic)
        oh["MajorLinkerVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MajorLinkerVersion)
        oh["MinorLinkerVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MinorLinkerVersion)
        oh["SizeOfCode"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfCode)
        oh["SizeOfInitializedData"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfInitializedData)
        oh["SizeOfUninitializedData"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfUninitializedData)
        oh["AddressOfEntryPoint"]=self._normalize(pelib.OPTIONAL_HEADER.AddressOfEntryPoint)
        ...
        oh["SectionAlignment"]=self._normalize(pelib.OPTIONAL_HEADER.SectionAlignment)
        oh["FileAlignment"]=self._normalize(pelib.OPTIONAL_HEADER.FileAlignment)
        oh["MajorOperatingSystemVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MajorOperatingSystemVersion)
        oh["MinorOperatingSystemVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MinorOperatingSystemVersion)
        oh["MajorImageVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MajorImageVersion)
        oh["MinorImageVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MinorImageVersion)
        oh["MajorSubsystemVersion"]=self._normalize(pelib.OPTIONAL_HEADER.MajorSubsystemVersion)
        ...
        oh["SizeOfHeaders"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfHeaders)
        oh["CheckSum"]=self._normalize(pelib.OPTIONAL_HEADER.CheckSum)
        oh["Subsystem"]=self._normalize(pelib.OPTIONAL_HEADER.Subsystem)
        oh["DllCharacteristics"]=self._normalize(pelib.OPTIONAL_HEADER.DllCharacteristics)
        oh["SizeOfStackReserve"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfStackReserve)
        oh["SizeOfStackCommit"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfStackCommit)
        oh["SizeOfHeapReserve"]=self._normalize(pelib.OPTIONAL_HEADER.SizeOfHeapReserve)
