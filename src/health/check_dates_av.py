...
def fix_date(r):
    ...
    if type(str_date) is not unicode:
        print "datenotunicode,"+str(r.get('_id'))+","+str(r.get('sha1'))+","+str(r.get('date'))+","+str(type(r.get('date')))
        sys.stdout.flush()
        return False
    else:
        try:
...
def main():
    ...
    fixed = 0
    print_flag = 1000000
    res = collection.find({},{"date": 1, "sha1": 1,"first_seen": 1},no_cursor_timeout=True).skip(start)
    for r in res:
        count += 1
        test += 1
