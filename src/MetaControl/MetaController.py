...
class MetaController():
    ...
    def searchExactImport(self, import_name, dll_name):
        r = self.import_coll.find_one(
            {"function_name": import_name, "dll_name": dll_name})
        return r

    def count_section_used(self, section_sha1):
        f = self.collection.find(
            {"particular_header.sections.sha1": section_sha1}).count()
        return f

    def count_resources_used(self, resources_sha1):
        f = self.collection.find(
            {"particular_header.res_entries.sha1": resources_sha1}).count()
        return f
