    ...
    while(try_again):
        try:
            ...
            if(fail_count >= 3):
                break
    if(response is None):
        key_manager.log_operation_result(
            apikey.get('key'), "download_sample", 'error')
...
def parse_vt_response(json_response):
    ...
    if json_response.get('additional_info') is not None and json_response.get('additional_info').get('imports') is not None:
        json_response["additional_info"]["imports"] = key_list_clean(
            json_response["additional_info"]["imports"])
    if json_response.get('additional_info') is not None and json_response.get('additional_info').get('pe-resource-types') is not None:
        json_response["additional_info"]["pe-resource-types"] = key_list_clean(
            json_response["additional_info"]["pe-resource-types"])
    ...
    try:
        ...
        ret["date"] = None
        logging.exception(
            "virusTotalApi->parse_vt_response: invalid date recieved by VT: " + str(vt_date))
...
def get_vt_av_result(file_id, priority="low"):
    ...
    try:
        response = requests.get(
            'https://www.virustotal.com/vtapi/v2/file/report', params=params, timeout=30)
    except Exception, e:
        logging.exception("VT /report request. " + str(e))
        key_manager.log_operation_result(
            apikey.get('key'), "av_analysis", 'error')
    ...
    if response.status_code == 200:
        try:
            ...
            logging.exception(
                "response.json() error. get_vt_av_result(" + str(file_id) + ")" + "e=" + str(e))
        ...
        key_manager.log_operation_result(
            apikey.get('key'), "av_analysis", 'invalid_key')
